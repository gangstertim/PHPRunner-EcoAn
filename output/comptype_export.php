...
$layout->blocks["top"][] = "export";$page_layouts["comptype_export"] = $layout;
...
$xt->display("comptype_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=comptype.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=comptype.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=comptype.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["DetailsEng"] = $pageObject->showDBValue("DetailsEng", $row);
			$values["detailsLL"] = $pageObject->showDBValue("detailsLL", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=comptype.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["DetailsEng"] = $pageObject->getViewControl("DetailsEng")->showDBValue($row, "");
			$values["detailsLL"] = $pageObject->getViewControl("detailsLL")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DetailsEng"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["detailsLL"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Details Eng").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Details LL").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Details Eng"."</td>";
		echo "<td>"."Details LL"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DetailsEng"]);
					else
						echo $values["DetailsEng"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["detailsLL"]);
					else
						echo $values["detailsLL"];
			echo '</td>';
							echo '<td>';
