...
include("include/mix_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["mix_search"] = $layout;
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("UnitOrBucket"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("UnitOrBucket")] = GetTableURL($pageObject->pSet->getLookupTable("UnitOrBucket"));
	
	$pageObject->fillFieldToolTips("UnitOrBucket");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("UnitOrBucket");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "UnitOrBucket";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("UnitOrBucket_label","<label for=\"".GetInputElementId("UnitOrBucket", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("UnitOrBucket_label", true);
	
	$xt->assign("UnitOrBucket_fieldblock", true);
	$xt->assignbyref("UnitOrBucket_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("UnitOrBucket_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("UnitOrBucket_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_UnitOrBucket", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("UnitOrBucket");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"UnitOrBucket", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"UnitOrBucket", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
