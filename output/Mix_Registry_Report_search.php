...
include("include/Mix_Registry_Report_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Mix_Registry_Report_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Mix_Registry_Report_search.htm";
...
$params['shortTableName'] = 'Mix_Registry_Report';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("MixColor"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("MixColor")] = GetTableURL($pageObject->pSet->getLookupTable("MixColor"));
	
	$pageObject->fillFieldToolTips("MixColor");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("MixColor");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "MixColor";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("MixColor_label","<label for=\"".GetInputElementId("MixColor", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("MixColor_label", true);
	
	$xt->assign("MixColor_fieldblock", true);
	$xt->assignbyref("MixColor_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("MixColor_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("MixColor_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_MixColor", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("MixColor");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MixColor", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MixColor", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
