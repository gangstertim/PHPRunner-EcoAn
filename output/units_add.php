...
$layout->blocks["top"][] = "details";$page_layouts["units_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "units_inline_add.htm";
else
	$templatefile = "units_add.htm";
...
//	processing Unit_LL - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Unit_LL = $pageObject->getControl("Unit_LL", $id);
		$control_Unit_LL->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Unit_LL - end
//	processing Unit_English_Full - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Unit_English_Full = $pageObject->getControl("Unit_English_Full", $id);
		$control_Unit_English_Full->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Unit_English_Full - end
//	processing Unit_LL_Full - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Unit_LL_Full = $pageObject->getControl("Unit_LL_Full", $id);
		$control_Unit_LL_Full->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Unit_LL_Full - end
//	processing Unit_Type_English - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Unit_Type_English = $pageObject->getControl("Unit_Type_English", $id);
		$control_Unit_Type_English->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Unit_Type_English - end
//	processing Rel_to_Base - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Rel_to_Base = $pageObject->getControl("Rel_to_Base", $id);
		$control_Rel_to_Base->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'units_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'units_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: units_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["UnitEng"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["UnitEng"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["Unit_LL"]=@$avalues["Unit_LL"];
	$defvalues["Unit_English_Full"]=@$avalues["Unit_English_Full"];
	$defvalues["Unit_LL_Full"]=@$avalues["Unit_LL_Full"];
	$defvalues["Unit_Type_English"]=@$avalues["Unit_Type_English"];
	$defvalues["Rel_to_Base"]=@$avalues["Rel_to_Base"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_LL_label","<label for=\"".GetInputElementId("Unit_LL", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_English_Full_label","<label for=\"".GetInputElementId("Unit_English_Full", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_LL_Full_label","<label for=\"".GetInputElementId("Unit_LL_Full", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_Type_English_label","<label for=\"".GetInputElementId("Unit_Type_English", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Rel_to_Base_label","<label for=\"".GetInputElementId("Rel_to_Base", $id, PAGE_ADD)."\">","</label>");
