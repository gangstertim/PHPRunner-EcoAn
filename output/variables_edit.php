...
$layout->blocks["top"][] = "details";$page_layouts["variables_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "variables_inline_edit.htm" : "variables_edit.htm";
...
//	processing ProdGrp - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ProdGrp = $pageObject->getControl("ProdGrp", $id);
		$control_ProdGrp->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ProdGrp - end
	...
	//	processing Status - end
			//	processing ProdGrp - begin
				...
				if($condition)
				{
					$control_ProdGrp->afterSuccessfulSave();
				}
	//	processing ProdGrp - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: variables_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: variables_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("ProdGrp"))
		$xt->assign("ProdGrp_fieldblock",true);
	else
		$xt->assign("ProdGrp_tabfieldblock",true);
	$xt->assign("ProdGrp_label",true);
	if(isEnableSection508())
		$xt->assign_section("ProdGrp_label","<label for=\"".GetInputElementId("ProdGrp", $id, PAGE_EDIT)."\">","</label>");
...
//	ProdGrp - 
	$value = $pageObject->showDBValue("ProdGrp", $data, $keylink);
	$showValues["ProdGrp"] = $value;
	$showFields[] = "ProdGrp";
		$showRawValues["ProdGrp"] = substr($data["ProdGrp"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='variables_view.php?".$viewlink."'\"");
