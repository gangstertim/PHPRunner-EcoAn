...
$layout->blocks["top"][] = "export";$page_layouts["productcolorcostjg_export"] = $layout;
...
$xt->display("productcolorcostjg_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=productcolorcostjg.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=productcolorcostjg.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=productcolorcostjg.xml");
	...
	{
		$values = array();
			$values["ProductColor"] = $pageObject->showDBValue("ProductColor", $row);
			$values["ProdGrpCode"] = $pageObject->showDBValue("ProdGrpCode", $row);
			$values["Cost4"] = $pageObject->showDBValue("Cost4", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=productcolorcostjg.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["ProductColor"] = $pageObject->getViewControl("ProductColor")->showDBValue($row, "");
			$values["ProdGrpCode"] = $pageObject->getViewControl("ProdGrpCode")->showDBValue($row, "");
			$values["Cost4"] = $row["Cost4"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ProductColor"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ProdGrpCode"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Cost4"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Product Color").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Prod Grp Code").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cost4").'</td>';	
	}
	else
	{
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["ProductColor"] = $pageObject->getViewControl("ProductColor")->showDBValue($row, "");
					$values["ProdGrpCode"] = $pageObject->getViewControl("ProdGrpCode")->showDBValue($row, "");
					$values["Cost4"] = $pageObject->getViewControl("Cost4")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ProductColor"]);
					else
						echo $values["ProductColor"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
