...
$layout->blocks["top"][] = "search";$page_layouts["unittype_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "unittype_search.htm";
...
$params['shortTableName'] = 'unittype';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Unit_Type_LL"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Unit_Type_LL")] = GetTableURL($pageObject->pSet->getLookupTable("Unit_Type_LL"));
	
	$pageObject->fillFieldToolTips("Unit_Type_LL");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Unit_Type_LL");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Unit_Type_LL";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_Type_LL_label","<label for=\"".GetInputElementId("Unit_Type_LL", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Unit_Type_LL_label", true);
	
	$xt->assign("Unit_Type_LL_fieldblock", true);
	$xt->assignbyref("Unit_Type_LL_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Unit_Type_LL_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Unit_Type_LL_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Unit_Type_LL", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Unit_Type_LL");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Unit_Type_LL", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Unit_Type_LL", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
