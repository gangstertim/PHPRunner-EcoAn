...
$layout->blocks["top"][] = "pdf";$page_layouts["buckets_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["BucketNo"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$arr = array();
$arr['fName'] = "DryWeight";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("DryWeight");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "BucketNo";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("BucketNo");
$fieldsArr[] = $arr;
...
//	DryWeight - Number
			$record["DryWeight_value"] = $pageObject->showDBValue("DryWeight", $data, $keylink);
			$record["DryWeight_class"] = $pageObject->fieldClass("DryWeight");
//	BucketNo - 
			$record["BucketNo_value"] = $pageObject->showDBValue("BucketNo", $data, $keylink);
			$record["BucketNo_class"] = $pageObject->fieldClass("BucketNo");
			if($col<$colsonpage)
...
$xt->assign("DryWeight_fieldheadercolumn",true);
$xt->assign("DryWeight_fieldheader",true);
$xt->assign("DryWeight_fieldcolumn",true);
$xt->assign("DryWeight_fieldfootercolumn",true);
$xt->assign("BucketNo_fieldheadercolumn",true);
$xt->assign("BucketNo_fieldheader",true);
$xt->assign("BucketNo_fieldcolumn",true);
$xt->assign("BucketNo_fieldfootercolumn",true);
