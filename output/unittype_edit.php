...
include("include/unittype_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["unittype_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "unittype_inline_edit.htm" : "unittype_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["Unit_Type_English"] = urldecode(postvalue("editid1"));
$savedKeys["Unit_Type_English"] = urldecode(postvalue("editid1"));
...
//	processing Unit_Type_English - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_Unit_Type_English->getWebValue()!==false)
			$keys["Unit_Type_English"] = $control_Unit_Type_English->getWebValue();
	}
//	processing Unit_Type_English - end
//	processing Unit_Type_LL - begin
	$condition = 1;

	if($condition)
	{
		$control_Unit_Type_LL = $pageObject->getControl("Unit_Type_LL", $id);
		$control_Unit_Type_LL->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Unit_Type_LL - end
	...
	//	processing Unit_Type_English - end
			//	processing Unit_Type_LL - begin
				...
				if($condition)
				{
					$control_Unit_Type_LL->afterSuccessfulSave();
				}
	//	processing Unit_Type_LL - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["Unit_Type_English"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: unittype_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: unittype_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Unit_Type_LL"] = $evalues["Unit_Type_LL"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Unit_Type_LL"))
		$xt->assign("Unit_Type_LL_fieldblock",true);
	else
		$xt->assign("Unit_Type_LL_tabfieldblock",true);
	$xt->assign("Unit_Type_LL_label",true);
	if(isEnableSection508())
		$xt->assign_section("Unit_Type_LL_label","<label for=\"".GetInputElementId("Unit_Type_LL", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("Unit_Type_English", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["Unit_Type_English"]));
...
//	Unit_Type_LL - 
	$value = $pageObject->showDBValue("Unit_Type_LL", $data, $keylink);
	$showValues["Unit_Type_LL"] = $value;
	$showFields[] = "Unit_Type_LL";
		$showRawValues["Unit_Type_LL"] = substr($data["Unit_Type_LL"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "unittype";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "unittype";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='unittype_view.php?".$viewlink."'\"");
