...
$layout->blocks["top"][] = "export";$page_layouts["Fixed_Components_Report_export"] = $layout;
...
$xt->display("Fixed_Components_Report_export.htm");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Fixed_Components_Report.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["(T2.Cost *T1.Qty )"] = $pageObject->showDBValue("(T2.Cost *T1.Qty )", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Fixed_Components_Report.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["(T2.Cost *T1.Qty )"] = $row["(T2.Cost *T1.Qty )"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["(T2.Cost *T1.Qty )"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("(T2.Cost *T1.Qty )").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["Qty"] = $pageObject->getViewControl("Qty")->showDBValue($row, "");
					$values["(T2.Cost *T1.Qty )"] = $pageObject->getViewControl("(T2.Cost *T1.Qty )")->showDBValue($row, "");
					$values["UnitEng"] = $pageObject->getViewControl("UnitEng")->showDBValue($row, "");
