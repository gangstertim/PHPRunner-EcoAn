...
$layout->blocks["top"][] = "export";$page_layouts["componentusage_export"] = $layout;
...
$xt->display("componentusage_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=componentusage.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=componentusage.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=componentusage.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["ComponentQty"] = $pageObject->showDBValue("ComponentQty", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=componentusage.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["ComponentQty"] = $row["ComponentQty"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ComponentQty"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Component Qty").'</td>';	
	}
	else
	{
		echo "<td>"."Prod No"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["ProdNo"] = $pageObject->getViewControl("ProdNo")->showDBValue($row, "");
					$values["ComponentNo"] = $pageObject->getViewControl("ComponentNo")->showDBValue($row, "");
					$values["ComponentQty"] = $pageObject->getViewControl("ComponentQty")->showDBValue($row, "");
