...
include("include/componentusage_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["componentusage_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "componentusage_search.htm";
...
$params['shortTableName'] = 'componentusage';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("ComponentQty"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ComponentQty")] = GetTableURL($pageObject->pSet->getLookupTable("ComponentQty"));
	
	$pageObject->fillFieldToolTips("ComponentQty");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ComponentQty");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "ComponentQty";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("ComponentQty_label","<label for=\"".GetInputElementId("ComponentQty", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ComponentQty_label", true);
	
	$xt->assign("ComponentQty_fieldblock", true);
	$xt->assignbyref("ComponentQty_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ComponentQty_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ComponentQty_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ComponentQty", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ComponentQty");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ComponentQty", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ComponentQty", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
