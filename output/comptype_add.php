...
$layout->blocks["top"][] = "details";$page_layouts["comptype_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "comptype_inline_add.htm";
else
	$templatefile = "comptype_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: comptype_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing CompTypeEng - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CompTypeEng = $pageObject->getControl("CompTypeEng", $id);
		$control_CompTypeEng->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CompTypeEng - end
//	processing CompTypeLL - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CompTypeLL = $pageObject->getControl("CompTypeLL", $id);
		$control_CompTypeLL->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CompTypeLL - end
//	processing DetailsEng - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DetailsEng = $pageObject->getControl("DetailsEng", $id);
		$control_DetailsEng->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DetailsEng - end
//	processing detailsLL - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_detailsLL = $pageObject->getControl("detailsLL", $id);
		$control_detailsLL->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'comptype_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'comptype_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: comptype_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["CompTypeNo"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["CompTypeNo"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["CompTypeEng"]=@$avalues["CompTypeEng"];
	$defvalues["CompTypeLL"]=@$avalues["CompTypeLL"];
	$defvalues["DetailsEng"]=@$avalues["DetailsEng"];
	$defvalues["detailsLL"]=@$avalues["detailsLL"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CompTypeEng_label","<label for=\"".GetInputElementId("CompTypeEng", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("CompTypeLL_label","<label for=\"".GetInputElementId("CompTypeLL", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DetailsEng_label","<label for=\"".GetInputElementId("DetailsEng", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("detailsLL_label","<label for=\"".GetInputElementId("detailsLL", $id, PAGE_ADD)."\">","</label>");
