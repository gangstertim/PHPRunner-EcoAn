...
add_nocache_headers();
include("include/buckets_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["buckets_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "buckets_inline_add.htm";
else
	$templatefile = "buckets_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing BucketNo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_BucketNo = $pageObject->getControl("BucketNo", $id);
		$control_BucketNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing BucketNo - end
...
//	processing DryWeight - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DryWeight = $pageObject->getControl("DryWeight", $id);
		$control_DryWeight->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DryWeight - end
...
//	processing BucketNo - start
			...
			if($inlineAddOption)
			{
				$control_BucketNo->afterSuccessfulSave();
			}
//	processing BucketNo - end
//	processing UnitEng - start
			$inlineAddOption = true;
...
//	processing DryWeight - start
			...
			if($inlineAddOption)
			{
				$control_DryWeight->afterSuccessfulSave();
			}
//	processing DryWeight - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'buckets_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'buckets_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: buckets_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["BucketNo"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["BucketNo"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["DryWeight"]=@$avalues["DryWeight"];
	$defvalues["BucketNo"]=@$avalues["BucketNo"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("DryWeight"))
		$xt->assign("DryWeight_fieldblock",true);
	else
		$xt->assign("DryWeight_tabfieldblock",true);
	$xt->assign("DryWeight_label",true);
	if(isEnableSection508())
		$xt->assign_section("DryWeight_label","<label for=\"".GetInputElementId("DryWeight", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("BucketNo"))
		$xt->assign("BucketNo_fieldblock",true);
	else
		$xt->assign("BucketNo_tabfieldblock",true);
	$xt->assign("BucketNo_label",true);
	if(isEnableSection508())
		$xt->assign_section("BucketNo_label","<label for=\"".GetInputElementId("BucketNo", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["BucketNo"]));
...
//	DryWeight
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("DryWeight", $data, $keylink);
		$showValues["DryWeight"] = $value;
		$showFields[] = "DryWeight";
	}	
//	BucketNo
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("BucketNo", $data, $keylink);
		$showValues["BucketNo"] = $value;
		$showFields[] = "BucketNo";
	}	
		$showRawValues["UnitEng"] = substr($data["UnitEng"],0,100);
		$showRawValues["Volume"] = substr($data["Volume"],0,100);
		$showRawValues["DryWeight"] = substr($data["DryWeight"],0,100);
		$showRawValues["BucketNo"] = substr($data["BucketNo"],0,100);
