...
$layout->blocks["top"][] = "details";$page_layouts["buckets_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["BucketNo"] = urldecode(postvalue("editid1"));
$savedKeys["BucketNo"] = urldecode(postvalue("editid1"));
...
//	processing DryWeight - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_DryWeight = $pageObject->getControl("DryWeight", $id);
		$control_DryWeight->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DryWeight - end
//	processing BucketNo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_BucketNo = $pageObject->getControl("BucketNo", $id);
		$control_BucketNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_BucketNo->getWebValue()!==false)
			$keys["BucketNo"] = $control_BucketNo->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["BucketNo"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: buckets_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: buckets_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["DryWeight"] = $evalues["DryWeight"];
	$data["BucketNo"] = $evalues["BucketNo"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("DryWeight_label","<label for=\"".GetInputElementId("DryWeight", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("BucketNo_label","<label for=\"".GetInputElementId("BucketNo", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("BucketNo", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='buckets_view.php?".$viewlink."'\"");
