...
$layout->blocks["top"][] = "details";$page_layouts["unittype_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "unittype_inline_add.htm";
else
	$templatefile = "unittype_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: unittype_add.php');
...
//	processing Unit_Type_LL - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Unit_Type_LL = $pageObject->getControl("Unit_Type_LL", $id);
		$control_Unit_Type_LL->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'unittype_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'unittype_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: unittype_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["Unit_Type_English"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["Unit_Type_English"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["Unit_Type_LL"]=@$avalues["Unit_Type_LL"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Unit_Type_LL_label","<label for=\"".GetInputElementId("Unit_Type_LL", $id, PAGE_ADD)."\">","</label>");
