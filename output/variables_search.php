...
include("include/variables_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["variables_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "variables_search.htm";
...
$params['shortTableName'] = 'variables';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("ProdGrp"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ProdGrp")] = GetTableURL($pageObject->pSet->getLookupTable("ProdGrp"));
	
	$pageObject->fillFieldToolTips("ProdGrp");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ProdGrp");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "ProdGrp";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("ProdGrp_label","<label for=\"".GetInputElementId("ProdGrp", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ProdGrp_label", true);
	
	$xt->assign("ProdGrp_fieldblock", true);
	$xt->assignbyref("ProdGrp_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ProdGrp_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ProdGrp_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ProdGrp", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ProdGrp");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ProdGrp", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ProdGrp", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
